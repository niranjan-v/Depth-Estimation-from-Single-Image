...
crit = nn.MSELoss()
optimizer = torch.optim.Adam(model.parameters(),lr = 2*1e-4,weight_decay=1e-4)
...
for ep in range(epochs) :
    ...
    # if (ep+1) % 10 == 0 :
    #   print("Model Saved")
    #   torch.save(torch.FloatTensor([valLoss,trainLoss]),"../bins/residual_apollo/residual_loss_A_temp.bin")
    #   torch.save(model.state_dict(),"../bins/residual_apollo/BestResidualmodel1_A_temp.pt")
...
### UNCOMMENT TO STORE MODEL
# torch.save(torch.FloatTensor([valLoss,trainLoss]),"../bins/residual_apollo/residual_loss_A.bin")

# torch.save(model.state_dict(),"../bins/residual_apollo/BestResidualmodel1_A.pt")
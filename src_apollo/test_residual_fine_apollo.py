...
model_fine_temp.load_state_dict(torch.load("../bins/residual_fine_apollo/BestResidualmodelFine1_A.pt"))
crit = nn.MSELoss()
...
for i in range(numBatches):
  output = model_fine_temp(test[batchSize*(i):batchSize*(i+1)].reshape(batchSize,3,226,282).float().cuda())
  output_2 = model_temp(test[batchSize*(i):batchSize*(i+1)].reshape(batchSize,3,226,282).float().cuda())
  for j in range(batchSize):
    output_j = (output[j]).reshape([57,71])
    output_2_j = (output_2[j]).reshape([57,71])
    ...
    ax4.imshow(ans[batchSize*i+j].reshape(57,71),cmap='hot')
    plt.show()
